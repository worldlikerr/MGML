...
class ConvLayer3D(nn.Module):
    def __init__(self, in_dim, out_dim, kernel_size=3, stride=1, padding=0, dilation=1, groups=1, norm=nn.BatchNorm3d, act_layer=nn.ReLU, bn_weight_init=1):
        super().__init__()
        self.conv = nn.Conv3d(
            in_dim,
            out_dim,
...
class FFN3D(nn.Module):
    def __init__(self, in_dim, dim):
        super().__init__()
        self.fc1 = ConvLayer3D(in_dim, dim, 1)
        self.fc2 = ConvLayer3D(dim, in_dim, 1, act_layer=None, bn_weight_init=0)
