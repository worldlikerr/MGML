...
def reshape_tokens_back(tokens, ref_feat_shape, num_modalities=1):
    """
    tokens: [B, N, C]
    ref_feat_shape: [B, C, D, H, W] (single modality)
    num_modalities: number of modalities to stack
    return: [B, C×M, D, H, W]
    """
    B, N, C = tokens.shape
    _, C_single, D, H, W = ref_feat_shape
    total_tokens = D * H * W * num_modalities
    assert N == total_tokens, f"Token数 {N} 不等于 {D}*{H}*{W}*{num_modalities} = {total_tokens}"
    tokens = tokens.view(B, num_modalities, D, H, W, C).permute(0, 1, 5, 2, 3, 4).contiguous()
    tokens = tokens.view(B, num_modalities * C, D, H, W)
    return tokens
...
class Model(nn.Module):
    ...
    def forward(self, x, mask):
        ...
        flair_intra_token_x5 = self.flair_transformer(flair_token_x5, self.flair_pos[:, :flair_token_x5.size(1), :])
        t1ce_intra_token_x5 = self.t1ce_transformer(t1ce_token_x5, self.t1ce_pos[:, :t1ce_token_x5.size(1), :])
        t1_intra_token_x5 = self.t1_transformer(t1_token_x5, self.t1_pos[:, :t1_token_x5.size(1), :])
        t2_intra_token_x5 = self.t2_transformer(t2_token_x5, self.t2_pos[:, :t2_token_x5.size(1), :])

        flair_intra_x5 = reshape_tokens_back(flair_intra_token_x5, flair_x5.shape)
        t1ce_intra_x5 = reshape_tokens_back(t1ce_intra_token_x5, t1ce_x5.shape)
        t1_intra_x5 = reshape_tokens_back(t1_intra_token_x5, t1_x5.shape)
        t2_intra_x5 = reshape_tokens_back(t2_intra_token_x5, t2_x5.shape)

        if self.is_training:
            flair_pred,flair_logits = self.decoder_sep(flair_x1, flair_x2, flair_x3, flair_x4, flair_x5)
            t1ce_pred, t1ce_logits = self.decoder_sep(t1ce_x1, t1ce_x2, t1ce_x3, t1ce_x4, t1ce_x5)
            t1_pred, t1_logits = self.decoder_sep(t1_x1, t1_x2, t1_x3, t1_x4, t1_x5)
            t2_pred, t2_logits = self.decoder_sep(t2_x1, t2_x2, t2_x3, t2_x4, t2_x5)
        ########### IntraFormer
        ...
        multimodal_pos = torch.cat((
            self.flair_pos[:, :flair_token_x5.size(1), :],
            self.t1ce_pos[:, :t1ce_token_x5.size(1), :],
            self.t1_pos[:, :t1_token_x5.size(1), :],
            self.t2_pos[:, :t2_token_x5.size(1), :]
        ), dim=1)
        ...
        multimodal_inter_x5 = self.multimodal_decode_conv(
            reshape_tokens_back(multimodal_inter_token_x5, flair_x5.shape, num_modalities=num_modals)
        )
        ...
        xx4 = torch.stack((flair_x4, t1ce_x4, t1_x4, t2_x4), dim=1)
        # print("xx4.shape:", xx4.shape)

        if self.is_training:
            return fuse_pred, (flair_pred, t1ce_pred, t1_pred, t2_pred), preds, (flair_logits, t1ce_logits, t1_logits, t2_logits),xx4
        return fuse_pred
...
if __name__ == '__main__':
    ...
    parser.add_argument('--datapath', default=r"D:\data\BRATS2020_Training_none_npy\BRATS2020_Training_none_npy", type=str)
    parser.add_argument('--dataname', default='BRATS2020', type=str)
    parser.add_argument('-batch_size', '--batch_size', default=1, type=int, help='Batch size')
    parser.add_argument('--pretrain', default=r"C:\Users\death\Documents\GitHub\python\Research\model_save\ours\metagss_selfloss\model_300_900_900.pth", type=str)
    ...
    train_set = Brats_loadall_nii(transforms=args.train_transforms, root=args.datapath, num_cls=4,
